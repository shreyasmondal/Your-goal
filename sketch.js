...
function preload() {
	...
	//final--------------------------------------------------------------------------------------------------------------->
	trophy_img = loadImage("Character/trophy.png")
	...
	//int---------------------------------------------------------------------------------------------------------------->
    int1 = loadImage("Character/intro/level_1_int.jpg")
	int2 = loadImage("character/intro/level_2_int.jpg")
	int3 = loadImage("Character/intro/level_3_int.jpg")
	int4 = loadImage("Character/intro/level_3.0_int.jpg")
	lostBackground = loadImage("Character/intro/lost.jpg")

	//sounds--------------------------------------------------------------------------------------------------------------->
	overSound = loadSound("Sounds/gameOver.mp3")
	winSound = loadSound("Sounds/finished.mp3")
	winTaDa = loadSound("Sounds/ta da.mp3")
}
...
function setup() {
    ...
    trophy = createSprite(character.x - 5, 270, 10, 10)
...
function draw() {
	...
	//Level 1_int------------------------------------------------------------------------------------------------------------->
	if (gameState === "level1int") {
		background(int1)
		character.visible = false

		if (frameCount%60===0 && intCount > 0) {
			intCount = intCount - 1
		}

		if (intCount === 0||keyDown("space")||keyDown("enter")) {
			gameState = "level1"
	...
	if (gameState === "level 1.1") {
		...
		stroke("white")
		textSize(20)
		text(timer1 + " seconds", character.x + 280, 160)
		...
		if (character.visible === true && obstaclesGroup.isTouching(character) || timer1 === 0) {
			gameOver()
			check = 1
		}
		...
		if (check < 1) {
			...
			if (frameCount % 60 === 0 && savedNotVisible === 0 && timer1 > 0) {
				timer1 = timer1 - 1
			}
	...
	//level change-------------------------------------------------------------------------------------------->
	if (ship.x > 8100 && levelD === 1 && gameLost === 0) {
		gameState = "level2.3.0int"
		msCount = 5
		character.scale = 2
		character.x = 400
		character.y = 466
	...
	if (ship.x > 8100 && levelD === 2 && gameLost === 0) {
		gameState = "level2.3.0int"
		msCount = 5
		character.scale = 2
		character.x = 400
...
function gameOver() {
    ...
    if (timer1 === 0) {
		text("Oh! The Ship Left Without You!!!", ship.x - 290, (displayHeight / 1.5) / 2 + 230)
		character.changeImage("st")
		if (ship.x<8250) {
			ship.velocityX = 4
		}else{
...
function won() {
	...
	trophy.x = character.x
	trophy.y = divider.y
	trophy.visible = true
	background("white")
	divider.destroy()
	finishLineSprite.destroy()
	character.destroy()
	enemy.destroy()
	winSound.play()
	winTaDa.play()
